...
class Item:
    def __init__(self, item_list):
        ...
        self.rap = item_list[2] if item_list[2] != 0 else 1_000_000
        self.value = item_list[3] if item_list[2] != 0 else 1_000_000
        self.default_value = int(item_list[4]) if item_list[2] != 0 else 1_000_000
...
def get_rand(amnt):
    ...
    for i in range(amnt):
        x.append(Item(items_list[item_choice[i-1]]))
...
def market():
    ...
    current_currency = get_money_from_db()
    return render_template('market.html', items=items, current_currency=get_money_from_db())

@app.route('/sell/<item_name>', methods=['POST', 'GET'])
def sell_item(item_name):
    conn = sqlite3.connect('items.db')
    c = conn.cursor()
    c.execute("SELECT rap FROM items WHERE item_name = ?", (item_name,))
